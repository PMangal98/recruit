...
<body>
    ...
    <div class="col col-12">
        <h2 class="main-head">Jobs:</h2>
        <div class="row p-3"> 
                <?php
                    $userId = $_SESSION['userId'];
                    $errors = array();
                    $query = "SELECT job_id,job_type, description, start_date, end_date, salary, location FROM job WHERE EXISTS (SELECT jobskill.skill_id FROM jobskill, employeeskill WHERE jobskill.job_id = job.job_id AND employeeskill.employee_id = {$userId} AND jobskill.duration < employeeskill.total_duration)";
                    ...
                    while($appl_row = mysqli_fetch_assoc($runQuery)){
                        echo '<div class="col col-md-6">
                            ...
                            <hr>
                            <button class="btn btn-rounded btn-success float-left" id="'.$appl_row["job_id"].'">Apply</button>
...
<script type="text/javascript">
    ...
    $(".btn").click(function(){
        ...
        var Info = {
            "jobId" : id,
            "eeId" : $_SESSION['userId'],
            "desc" : "I want this Job"
        }
        alert("HI");
        $.ajax({
            url: "./jobDecision.php",
